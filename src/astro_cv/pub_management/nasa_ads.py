...
def build_query(name: str, affiliations: list[str], orcid: str, started_year: int, false_lib_id: str) -> str:
...
def obtain_query_papers(query: str, name: str, affiliations: list[str], orcid: str, false_lib: Library):
    ...
    mask = [not affs or any(any(a in aff for a in affiliations) for aff in affs) for affs in author_aff]
    papers = [p for m, p in zip(mask, papers) if m]
    indices = [p for m, p in zip(mask, indices) if m]
    author_aff = [p for m, p in zip(mask, author_aff) if m]
    orcids = [p for m, p in zip(mask, orcids) if m]
...
def obtain_library_papers(library: Library, exclude_orcid: str = "", extra_fields: tuple[str] | None = None):
    ...
    known_papers = list(ads.SearchQuery(
        q=query,
        fl=fields,
        sort='date desc',
        max_pages=20  # Reduced from 100 to prevent API timeouts
...
def compare_query_to_library(papers, known_papers, indices, author_aff, orcids):
    ...
    return already_in, new_papers, excess_papers, new_indices, new_author_aff, new_orcids
...
def add_new_papers_to_library(new_papers, new_orcids, new_author_aff, new_indices, library: Library, false_lib: Library):
...
def write_library_cache(
    papers: list, 
    cache_path: Path,
    has_citation_counts: bool = False
):
    ...
    if not has_citation_counts:
        ...
        if bibcodes:
            ...
            cited_papers = list(ads.SearchQuery(
    ...
    for paper in papers:
        ...
        if any(val is None for key, val in this.items()):
            raise ValueError(f"Warning: Paper {paper.bibcode} has missing fields.\n{this}")
