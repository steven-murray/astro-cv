...
from .datatypes import PubList, Publication
from .stats import (
    compute_h_index,
    compute_i10_index,
...
def report(
    ...
    if not cache.exists():
        console.print(f"[bold red]No library cache found at {cache}. Run [bold]astro-cv ads refresh-cache[/bold] to create one.[/bold red]")
        return
    ...
    console.print(f"[bold]Library Report[/bold]")
    ...
    console.print(f"h-index of library: [bold green]{compute_h_index(papers)}[/bold green]")
    ...
    console.print(f"i10-index of library: [bold green]{compute_i10_index(papers)}[/bold green]")
    
    console.print(f"Median citations per paper: [bold green]{median_citations(papers):.2f}[/bold green]")
    console.print(f"Average citations per paper: [bold green]{average_citations(papers):.2f}[/bold green]")
    console.print(f"Total citations: [bold green]{total_citations(papers)}[/bold green]")
    ...
    papers_per_journal = dict(sorted(papers_per_journal.items(), key=lambda item: item[1], reverse=True)[:10])
    ...
    for p in top_papers:
        console.print(f"  [[bold]{p.citation_count} citations[/bold]] {p.title[0]} [cyan]({p.year})[/cyan]")
    ...
    for p in top_papers_per_year:
        console.print(f"  [[bold]{p.citations_per_year:.1f}[/bold]] {p.title[0]} [cyan]({p.year})[/cyan]")
    ...
    if cached_library:
        ...
        for p in papers:
            ...
            if cached_paper and p.citation_count != oldcount:
                new_citations[p.bibcode] = (oldcount, p.citation_count, p.citation_count - oldcount)
        
        console.print(f"[bold]Comparing to previous snapshot at '{cached_library['date_compiled']}'[/bold]")
        console.print(f"[bold green]Total citation count changes:[/bold green] {sum(x[2] for x in new_citations.values())}")
        console.print(f"[bold green]Total papers with citation count changes:[/bold green] {len(new_citations)}")
        if new_citations:
            console.print(f"[bold green]Top 5 citation count increases:[/bold green]")
            top_increases = sorted(new_citations.items(), key=lambda x: x[1][2], reverse=True)[:5]
            for bibcode, (old_count, new_count, diff) in top_increases:
                console.print(f"  - [dim]{bibcode}:[/dim] {old_count} -> {new_count} ([green]+{diff}[/green])")

        new_papers = [p for p in papers if p.bibcode not in cachepubs]
        if new_papers:
            console.print(f"[bold green]New Papers added since last snapshot:[/bold green]")
            for p in new_papers:
                console.print(f"  - [dim]{p.bibcode}:[/dim] {p.title[0]} [cyan]({p.year})[/cyan]")
...
def manage(
    ...
    already_in, new_papers, excess_papers, new_indices, new_author_aff, new_orcids = compare_query_to_library(
        papers, known_papers, indices, author_aff, orcids
    )
    
    console.print(f"[bold green]Found {len(papers)} papers in the search.[/bold green] {len(already_in)} were already in your library. [bold yellow]{len(new_papers)} are new.[/bold yellow]")
    ...
    if papers_to_print:
        for p in papers_to_print:
            console.print(f"  [dim]{','.join(p.author)}:[/dim] {p.title[0]} [cyan]({p.year})[/cyan] / {p.aff} {p.orcid_pub}")
    
    if new_papers:
        console.print(f"[bold yellow]Found {len(new_papers)} new papers, please select whether each one should be in your library:[/bold yellow]")
    
    add_new_papers_to_library(new_papers, new_orcids, new_author_aff, new_indices, library, false_lib)
    ...
    if orcid_papers:
        console.print("[bold magenta]The following papers were in your library but are not associated with your ORCID on ADS:[/bold magenta]")
        for p in orcid_papers:
            console.print(f"  - [dim]{p.author[0]}:[/dim] {p.title[0]} [cyan]({p.year})[/cyan]")
        console.print("[bold]Follow this link to associate them:[/bold]")
        console.print(f'  [link=https://ui.adsabs.harvard.edu/search/q=docs(library%2F{library.id})%20NOT%20orcid%3A{cfg["orcid"]}]https://ui.adsabs.harvard.edu/search/q=docs(library%2F{library.id})%20NOT%20orcid%3A{cfg["orcid"]}[/link]')
