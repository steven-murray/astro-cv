...
APP_NAME = "astro-cv"
DEFAULT_AUTH_FILENAME = "authorization_key.json"


def default_auth_file() -> Path:
    """Return the default service account key location in user config dir."""
    return Path(user_config_dir(APP_NAME)) / DEFAULT_AUTH_FILENAME
...
class DataConnector:
    ...
    def __init__(
        ...
        Parameters
        ----------
        auth_file : Path or str or None
            Path to the service account JSON file for authentication.
            If None, defaults to appdirs config location.
        sheet_name : str
            Name of the Google Sheet to open.
        """
        self.auth_file = (
            default_auth_file() if auth_file is None else Path(auth_file).expanduser()
        )
    ...
    def _authenticate(self) -> None:
        ...
        try:
            ...
            raise FileNotFoundError(
                "Error: Authentication file not found at "
                f"{self.auth_file}. "
                "Place your Google service account JSON key there, or set "
